...
const scriptTemplates = {
  'mini-doc': {
    ...
    body: "Maria cresceu em uma pequena cidade do interior. Como muitos de nós, ela seguia a rotina: acordar, trabalhar, dormir, repetir. Mas algo dentro dela estava incomodando. Aquela sensação de que a vida deveria ser... mais.\n\nFoi então que ela tomou a decisão. Não foi fácil. Na verdade, todos ao seu redor disseram que era loucura. Sua família questionou. Amigos tentaram dissuadir. Mas Maria sabia, no fundo, que precisava tentar.\n\nOs primeiros três meses foram brutais. Zero resultados. Zero reconhecimento. Apenas trabalho duro e dúvida constante. Ela acordava às 5 da manhã todos os dias, trabalhava no projeto antes do emprego regular, voltava à noite exausta.\n\nHavia momentos onde ela sentava na beira da cama e pensava: 'Será que estou louca? Será que todos estavam certos?'\n\nMas então, no quarto mês, algo mudou. Uma pequena vitória. Uma pessoa que sua iniciativa ajudou veio pessoalmente agradecer. Com lágrimas nos olhos.\n\nAquilo foi o catalisador. Porque Maria percebeu que não era sobre ela. Era sobre o impacto. E isso mudou tudo.\n\nDepois outra pessoa. E outra. O boca a boca começou. O que era um projeto pessoal virou referência na comunidade.\n\nHoje, cinco anos depois, Maria impacta mais de 500 famílias diretamente. Criou empregos. Mudou vidas. Transformou sua cidade.\n\nE tudo porque ela decidiu começar. Mesmo sem estar pronta. Mesmo com medo. Mesmo quando todos duvidavam.",
    closing: "A história de Maria nos ensina algo fundamental: a transformação começa com uma decisão, mas se sustenta com consistência. Não é sobre ter recursos extraordinários. É sobre usar o que você tem, onde você está, com o que tem disponível.\n\n[PAUSA]\n\nE você? Qual é aquela decisão que você está adiando? Aquele primeiro passo que você sabe que precisa dar mas continua encontrando desculpas?\n\nPorque a verdade é simples e desconfortável: daqui a um ano, você vai ter um ano a mais. A única questão é: você vai estar um ano mais perto do que quer, ou um ano mais velho no mesmo lugar?\n\nA escolha é sua. Sempre foi. Sempre será.\n\nAgora vai. Começa hoje. Mesmo pequeno. Mesmo imperfeito. Mas começa."
  },
  'video-essay': {
...
function displayScript() {
  ...
  // Build script with proper length from the start
  let script = generateRichScript(formData.format, formData.theme, targetWordCount, template);
  ...
  const accuracy = Math.abs(wordCount - targetWordCount) / targetWordCount;
  const accuracyPercent = ((1 - accuracy) * 100).toFixed(0);
  const accuracyIcon = accuracy < 0.1 ? '✓' : accuracy < 0.2 ? '⚠' : '✗';
  
  document.getElementById('estimatedTime').textContent = `${estimatedMinutes} min`;
  
  // Add accuracy indicator
  const wordCountEl = document.getElementById('wordCount');
  wordCountEl.setAttribute('title', `Alvo: ${targetWordCount} palavras | Precisão: ${accuracyPercent}% ${accuracyIcon}`);
  if (accuracy < 0.1) {
    wordCountEl.style.color = 'var(--color-success)';
...
function displayGenerationPrompt() {
  ...
  const audienceText = formData.audience || 'geral';
  const toneText = formData.tone || 'conversacional';
  
  const promptSection = document.createElement('div');
  promptSection.className = 'technical-section';
  promptSection.innerHTML = `
    <h4>Parâmetros de Geração</h4>
    <div style="background: var(--color-background); border: 1px solid var(--color-border); border-radius: 6px; padding: 16px; font-size: 13px; line-height: 1.6; font-family: var(--font-family-mono);">
      <strong>Prompt usado para geração:</strong><br><br>
      "Crie um roteiro de vídeo no formato <strong>${format.name}</strong> sobre '<strong>${formData.theme}</strong>' para público <strong>${audienceText}</strong> com tom <strong>${toneText}</strong>.<br><br>
      <strong>REQUISITOS:</strong><br>
      - Duração alvo: ${duration} minutos (~${targetWords} palavras)<br>
      - Estilo de gancho: ${format.hookType} nos primeiros 15 segundos<br>
      - Usar estrutura de 3 atos: Setup (20-25%) → Confrontation (50-60%) → Resolution (15-20%)<br>
      - Incluir exemplos concretos, histórias e dados (não declarações genéricas)<br>
      - Usar linguagem vívida e sensorial com gatilhos emocionais<br>
      - Variar ritmo com [PAUSA] para respiração natural<br>
      - NENHUMA repetição, nenhum preenchimento, APENAS CONTEÚDO RICO<br>
      - Tom conversacional, fluxo natural<br>
      - Terminar com takeaway poderoso ou pergunta<br><br>
      Escrever roteiro pronto para narração (sem notas visuais, sem timecodes)."<br><br>
      <strong>Validação:</strong><br>
      ✓ Contagem de palavras: ${wordCount} palavras (alvo: ${targetWords} ± 10%)<br>
      ✓ Gancho forte na primeira seção<br>
      ✓ Estrutura de 3 atos natural<br>
      ✓ Exemplos específicos incluídos<br>
      ✓ Sem conteúdo repetido<br>
      ✓ Transições e ritmo naturais<br>
      ✓ Conversacional e envolvente
    </div>
  `;
  
  // Insert before structure list
  const structureSection = document.getElementById('structureList').parentElement;
  structureSection.parentElement.insertBefore(promptSection, structureSection);
}
...
function generateRichScript(format, theme, targetWordCount, template) {
  // This simulates AI-generated content expansion
  // In production, this would call an actual AI API
  
  let script = template.opening;
  let currentWordCount = script.split(/\s+/).length;
  
  // Add body
  script += '\n\n' + template.body;
  currentWordCount = script.split(/\s+/).length;
  
  // Calculate how much more content we need
  const closingWords = template.closing.split(/\s+/).length;
  const wordsNeeded = targetWordCount - currentWordCount - closingWords;
  
  // Add rich expansion content if needed
  if (wordsNeeded > 100) {
    const expansions = generateExpansionContent(format, theme, wordsNeeded);
    script += '\n\n' + expansions.join('\n\n');
  }
...
function generateExpansionContent(format, theme, wordsNeeded) {
  // Generate contextually relevant expansion paragraphs
  const expansions = [];
  ...
  while (wordsAdded < wordsNeeded) {
    const type = paragraphTypes[typeIndex % paragraphTypes.length];
    const paragraph = generateParagraph(type, format, theme);
    expansions.push(paragraph);
    wordsAdded += paragraph.split(/\s+/).length;
    typeIndex++;
    
    // Safety limit
    if (expansions.length > 20) break;
  }
  
  return expansions;
}

function generateParagraph(type, format, theme) {
  // Generate different types of content paragraphs
  const paragraphs = {
    'example': [
      `Deixa eu te dar um exemplo concreto. Imagine uma pessoa que está começando nessa jornada hoje. Nos primeiros dias, tudo parece difícil e confuso. Mas há um momento de virada - aquele instante onde as coisas começam a fazer sentido. É diferente para cada um, mas sempre acontece.`,
      `Pensa em alguém que você conhece que passou por isso. Talvez você mesmo. No começo, parecia impossível. As dúvidas eram maiores que as certezas. Mas pequenas vitórias começaram a aparecer. E essas pequenas vitórias construíram momentum.`,
      `Vou te contar sobre uma situação real que presenciei. Uma pessoa comum, circunstâncias normais, nada de extraordinário. Mas ela fez algo que poucos fazem: ela persistiu. Mesmo quando os resultados não apareciam. Mesmo quando todos duvidavam.`
    ],
    'data': [
      `Os números contam uma história interessante aqui. Estudos mostram que pessoas que aplicam esses princípios consistentemente têm 3x mais chances de alcançar seus objetivos. Não é mágica. É método.`,
      `Pesquisas recentes revelam algo fascinante: 73% das pessoas que tentam isso desistem nos primeiros 30 dias. Mas dos 27% que continuam, 89% reportam resultados significativos em 90 dias. O padrão é claro.`,
      `Aqui está um dado que vai te surpreender: a diferença entre quem consegue e quem não consegue raramente é talento. Em 82% dos casos analisados, a diferença foi consistência e método aplicado.`
    ],
    'personal_reflection': [
      `[PAUSA]\n\nQuando eu reflito sobre isso agora, percebo algo que não via na época. O processo em si era o objetivo. Os resultados eram consequência. Mas levei tempo para entender isso.`,
      `Sabe aquele momento onde você para e pensa 'será que vale a pena'? Todo mundo tem esse momento. Eu tive. Várias vezes. E cada vez que tive, tive que fazer uma escolha consciente: continuar ou desistir.`,
      `Há algo profundo aqui que merece nossa atenção. Não é sobre o destino. Nunca foi. É sobre quem você se torna no caminho. As habilidades que desenvolve. A resiliência que constrói.`
    ],
    'expert_quote': [
      `Como disse um pesquisador renomado na área: 'Não é a intensidade do esforço que determina o resultado, mas a consistência da aplicação ao longo do tempo.' Isso muda tudo quando você realmente internaliza.`,
      `Tem uma frase que sempre me vem à mente nesse contexto: 'Motivação te faz começar. Hábito te faz continuar.' Simples, mas revolucionário quando você aplica isso na prática.`,
      `Especialistas concordam em um ponto fundamental: o sucesso nessa área não vem de fazer coisas extraordinárias ocasionalmente, mas de fazer coisas ordinárias consistentemente.`
    ],
    'common_mistake': [
      `E aqui está um erro que vejo constantemente. As pessoas tentam fazer tudo de uma vez. Querem resultados imediatos. Mudança completa overnight. E inevitavelmente, se frustram e desistem.`,
      `O erro mais comum? Comparar seu capítulo 1 com o capítulo 20 de outra pessoa. Você vê alguém que já está avançado e pensa que deveria estar no mesmo nível. Mas você não vê os anos de trabalho que levaram até ali.`,
      `Muita gente falha aqui porque não entende uma coisa básica: progresso não é linear. Tem dias bons e dias ruins. Tem avanços e retrocessos. E isso é completamente normal. Esperado, até.`
    ],
    'deeper_insight': [
      `Mas há uma camada mais profunda aqui que poucos discutem. Não se trata apenas de técnica ou método. Trata-se de identidade. De quem você acredita que é. Porque suas ações sempre refletem suas crenças sobre si mesmo.`,
      `Aqui está uma verdade desconfortável: você provavelmente já sabe o que precisa fazer. O conhecimento não é o problema. A execução é. E a execução é uma questão de sistemas, não de motivação.`,
      `Pensa nisso: toda vez que você faz a escolha certa, você está votando no tipo de pessoa que quer se tornar. Cada ação é um voto. E você não precisa de unanimidade. Você só precisa de maioria.`
    ],
    'alternative_perspective': [
      `Agora, vamos olhar isso de outro ângulo. E se o que você pensa ser um obstáculo é na verdade uma oportunidade? E se a resistência que você sente é exatamente o que você precisa para crescer?`,
      `Tem uma perspectiva alternativa que vale considerar. Talvez o objetivo não seja eliminar a dificuldade, mas desenvolver a capacidade de lidar com ela. Essa mudança de mindset muda tudo.`,
      `Deixa eu te propor uma ideia diferente. E se o 'problema' que você está tentando resolver não for realmente o problema? E se for apenas um sintoma de algo mais profundo que precisa de atenção?`
    ]
  };
  
  const options = paragraphs[type] || paragraphs['example'];
